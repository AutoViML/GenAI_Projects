...
##############################################################################
# Vertex RAG Demo - Single LLM with Judge Model
...
FINAL_ANSWER_PROMPT_FILE = "final_answer.txt"
JUDGE_PROMPT_FILE = "judge_prompt.txt"
JUDGE_MODEL_NAME_FILE =  "judge_model_name.txt"
...
GEMINI_MODEL_PREFIXES = ["gemini-2.", "gemini-2.5"]
...
def get_final_answer_prompt(documents: str, query: str) -> str:
    """
    Loads and formats the summarizer prompt with retrieved documents.
    ...
    """
    try:
        prompt_template = _load_prompt_template(FINAL_ANSWER_PROMPT_FILE)

        # Define the dynamic part (query, docs) - adjust based on actual template needs
        formatted_prompt_body = f"""
        
Now it's your turn! Here is the query and relevant context:
Customer Search Query: {query}
...
"""
        ...
        return _apply_system_prompt(full_prompt)

    except KeyError as e:
        log_debug(f"Error formatting summarizer prompt: Missing key {e}")
        raise # Re-raise
    except Exception as e:
        log_debug(f"Unexpected error formatting summarizer prompt: {e}")
        raise # Re-raise
...
def generate_summarizer_response(
    ...
    """
    Orchestrates summarizer generation using the selected model and context.
    ...
    try:
        ...
        with col, st.chat_message("ai"):
             st.write(f"*{model_config.get('name', 'Model')} is generating the summarizer response...*")
        ...
        log_debug(f"Summarizer Response:\n {response_text}")

    except Exception as e:
        log_debug(f"Error generating summarizer response: {e}")
        st.error(f"Error generating summarizer response: {e}")
...
def generate_final_answer_response(
    ...
    try:
        log_debug(f"Generating final answer response with config: {model_config}")
        # Get formatted prompt (raises error on failure)
        final_answer_prompt = get_final_answer_prompt(context, question)
        ...
        if model_type == "gemini":
            response_text = generate_gemini_response(
                final_answer_prompt, model_config, col
            )
        elif model_type == "3p_models": # Changed from "3p_models"
            response_text = generate_ollama_response(
                final_answer_prompt, model_config, col
        ...
        log_debug(f"Error generating final answer response: {e}")
        st.error(f"Error generating final answer: {e}")
        error_message = str(e) # Capture error message

    elapsed_time = (time.time())*1000 - start_time
    log_debug(f"Final answer time: {elapsed_time:.1f} seconds")
    ...
    with column:
        ...
        if model_type == "3P_Models":
            ...
            if not available_models:
                 ...
                 model_name = "gemini-2.0-flash-001" # Fallback default
            else:
                default_index = 0
                try:
                    # Try to find a reasonable default like flash
                    default_index = available_models.index("gemini-2.0-flash-001")
                except ValueError:
...
def return_thinking_result(response):
  full_response = ""
  for part in response.candidates[0].content.parts:
    ...
    if part.code_execution_result is not None:
      full_response += part.code_execution_result.output
  return full_response
...
def judge_responses(left_question, left_response, left_context):
    """
    ...
    """
    prompt_template = _load_prompt_template(JUDGE_PROMPT_FILE)

    try:
        formatted_prompt = """Given the following QUESTION and the CONTEXT which is the source of truth to use, judge the model's response based on rubric given:
        
Question, context and response from model:
        ...
        """
        judge_prompt = prompt_template + formatted_prompt.format(left_question=left_question,
                left_context=left_context, left_response=left_response['text'])
    except KeyError as e:
        logging.error(f"Error formatting judge prompt: Missing key {e}")
        st.error("Internal error: Could not format the judge prompt.")
        # Load the prompt for the judge model
        judge_prompt = "You are an AI Bot who acts as a judge to analyze a response to a question based on the context given."
        
    # Initialize the Gemini model for judging ()
    model_name = _load_prompt_template(JUDGE_MODEL_NAME_FILE).replace("\n","") 
    ...
    generate_content_config = types.GenerateContentConfig(
            ...
            system_instruction=[types.Part.from_text(text=judge_prompt)],
          )
    try:
        response = "Error during generating judge_response"
        ...
        response = return_thinking_result(response)        
        log_debug("Judge response: %s " %response)
    except Exception as e:
        log_debug(f"Error during generating judge_response: {e}")
...
def main(args: argparse.Namespace): # <-- Pass parsed args to main
    ...
    st.set_page_config("Vertex Search RAG with multiple LLMs", layout="wide")
    st.title("📊 ContextualAI ISV Chatbot Demo")
    st.caption("ContextualAI ISV Chatbot Demo with Judge model")
    ...
    ## Since there is only one column, you have to still get the container from that list
    left_col = left_col[0]
    ...
    if user_question:
        ...
        # --- Step 5: Now is the time to add a final answer ---
        if rag_active:
            log_debug("Generating final answer for Left model using RAG...")
            left_response_input = left_response_data.get('text') # Use datastore if RAG is used
            left_final_response = generate_final_answer_response(
                left_config, left_final_query, left_response_input, left_col,
                )
        else:
            logger.info("Generating final answer for Left model without RAG ...")
            left_response_input = ""
            left_final_response = generate_final_answer_response(
                left_config, left_final_query, left_response_input, left_col,
                )
        ...
        st.session_state.chat_history.append(
            {
                ...
                "left_query_used": left_final_query, # Store the query actually used
                "left_context_used": bool(left_context), # Store if RAG context was provided
                "left_final_response": left_final_response,
                "timestamp": time.time(),
            }
        )
        
        # --- Step 6: Run Judge Model (Conditional) ---
        if args.judge: # <-- Check the command-line argument flag for judge model
            ...
            judgement = judge_responses(left_final_query, left_response_data, left_context) # function calls that get the judge response!
            ...
            with st.container():
                ...
                st.write(judgement) # add that line to print it to screen!            

##################### MAIN APP COMPLETE ##################################
if __name__ == "__main__":
    # --- Argument Parsing ---
    parser = argparse.ArgumentParser(description="Vertex RAG Demo with optional Judge Model.")
    parser.add_argument(
        "--judge",
